...
class Temple extends PopulatorObject{
	...
	public const DIRECTION_PLUSZ = 2;
	public const DIRECTION_MINZ  = 3;

	public const THREE_DIAGS = [
		[
			3,
			0
		],
	 ...
	 * @param int $meta
	 */
	protected function placeBlock(int $x, int $y, int $z, int $id = Block::SANDSTONE, int $meta = 0){
		$this->level->setBlockIdAt($x, $y, $z, $id);
		$this->level->setBlockDataAt($x, $y, $z, $meta);
	}

	/**
	 * Places one of the towers. Out is inversed $direction1, stairs come from inversed $direction2 to $direction2,
	 * patterns are on $direction1 and $direction2
	 *
	 * @param $x          int
	 * @param $y          int
	 * @param $z          int
	 ...
	 * @return void
	 */
	public function placeTower(int $x, int $y, int $z, int $direction1 = self::DIRECTION_PLUSX, int $direction2 = self::DIRECTION_PLUSZ){
